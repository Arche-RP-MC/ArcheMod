...
import fr.archemod.items.inventory.slot.SlotGrandSac;
...
public class ContainerGrandSac extends Container {
    public InventoryGrandSac invGrandSac;
    public int rows;

    public ContainerGrandSac(InventoryPlayer playerInv, InventoryGrandSac inv) {
        this.invGrandSac = inv;
        ...
        for (j = 0; j < this.rows; ++j) {
            for (k = 0; k < 6; ++k) {
                this.addSlotToContainer(new SlotGrandSac(inv, k + j * 6, 26 + k * 18, 18 + j * 18));
            }
        }
    ...
    public void writeToNBT(ItemStack stack) {
        ...
        invGrandSac.writeToNBT(stack.getTagCompound());
    }
    ...
    public ItemStack slotClick(int slotId, int dragType, ClickType clickTypeIn, EntityPlayer player) {
        ...
        if (slotId - this.invGrandSac.getSizeInventory() - 27 == player.inventory.currentItem) return ItemStack.EMPTY;
